...
serve(async (req) => {
  ...
  let sent=0;
  const { data: earned } = await db.from('referral_coupons').select('id,discount_percent,discount_amount_limit,owner:profiles!referral_coupons_owner_id_fkey(name,phone),referral_notification_logs(notification_type)').eq('status','available');
  for (const coupon of earned || []) {
    ...
    const credit = Number(coupon.discount_amount_limit ?? (Number(coupon.discount_percent || 0) * 25 / 100));
    const res = await fetch(`${url}/functions/v1/whatsapp-notify`, { method:'POST', headers:{'Content-Type':'application/json','apikey':legacyInvokeKey,Authorization:`Bearer ${legacyInvokeKey}`}, body:JSON.stringify({ appointmentId:coupon.id, clientName:owner.name, phone:owner.phone, action:'referral_earned', serviceName:`até R$ ${credit.toFixed(2)}` }) });
  ...
  const { data: coupons, error } = await db.from('referral_coupons').select('id,discount_percent,discount_amount_limit,expires_at,owner:profiles!referral_coupons_owner_id_fkey(name,phone)').eq('status','available').gte('expires_at',from).lt('expires_at',to);
