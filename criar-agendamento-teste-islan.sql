-- SQL para criar agendamento de teste para Islan Raimundo
-- Cliente: 96991944679
-- Este agendamento será criado com booking_type='online' para disparar notificações WhatsApp

-- 1. Buscar ID do barbeiro Islan Raimundo
-- SELECT id, name, whatsapp_phone FROM barbers WHERE name ILIKE '%Islan%';

-- 2. Buscar ID do primeiro serviço disponível
-- SELECT id, title FROM services WHERE visible = true ORDER BY order_index LIMIT 1;

-- 3. Criar ou buscar perfil do cliente
DO $$
DECLARE
  v_barber_id UUID;
  v_service_id UUID;
...
BEGIN
  ...
  -- Se for muito tarde no dia, agendar para amanhã
  IF v_appointment_time < CURRENT_TIME THEN
    v_appointment_date := CURRENT_DATE + INTERVAL '1 day';
    v_appointment_time := '10:00'::TIME; -- Horário inicial do dia seguinte
  END IF;
...
-- 5. Verificar se notificações foram criadas na fila
SELECT 
  id,
  appointment_id,
  client_name,
...
WHERE client_phone = '96991944679'
